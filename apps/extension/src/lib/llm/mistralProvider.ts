...
function headers(apiKey?: string): HeadersInit {
  if (!apiKey) throw new LlmError('CREDENTIAL_NOT_CONFIGURED', 'Mistral API key is not configured.')
  return { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }
}

export function createMistralProvider(config: ProviderRuntimeConfig): LlmProvider {
  return {
    id: 'mistral', label: 'Mistral', mode: 'cloud',
    async testConnection() {
      ...
      await requireOk(response, 'Mistral')
      return { available: true, label: 'Mistral', details: 'Connected.', model: config.model }
    },
    async generateFieldSuggestions(input) {
      ...
      await requireOk(response, 'Mistral')
      const data = CompletionSchema.parse(await readJson(response, 'Mistral'))
      const content = data.choices?.[0]?.message?.content
      if (!content) throw new LlmError('INVALID_JSON', 'Mistral returned an empty response.')
